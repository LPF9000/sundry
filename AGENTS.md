...
(`src/semiconductor_digest/`) is generic — it knows nothing about
semiconductors specifically. All topic-specific content (sources,
categories, keywords) lives in one file: `config/feeds.toml`.
...
- **No AI-authorship attribution** in commits, code comments, or
  rendered output (no "Generated by Claude" style footers in code —
  PR/commit-message footers per your own harness's rules are a separate,
  external concern and fine).
- **Source/category data belongs in `config/feeds.toml`, never in
  `src/`.** If a task seems to require editing `src/semiconductor_digest/`
